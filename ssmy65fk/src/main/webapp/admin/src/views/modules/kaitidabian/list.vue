<template>
	<div class="main-content" :style='{"padding":" 30px 20px","flexWrap":"wrap","background":"none","display":"flex"}'>
		...
		<template v-if="showFlag">
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					label="开题时间">
						<template slot-scope="scope">
							{{scope.row.kaitishijian}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"cursor":"pointer","padding":"0 8px 0 20px","margin":"0 4px 10px","borderColor":"#d7e1eb","color":"#1b89ef","outline":"none","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230220/b0c049479a6d42828661d85d56848f78.png) no-repeat 4px 8px,#ebf4fd","borderWidth":"1px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('kaitidabian','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 8px 0 20px","margin":"0 4px 10px","borderColor":"#d7e1eb","color":"#1b89ef","outline":"none","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230220/b0c049479a6d42828661d85d56848f78.png) no-repeat 4px 8px,#ebf4fd","borderWidth":"1px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('kaitidabian','答辩评审')" type="success" size="mini" @click="dabianpingshenCrossAddOrUpdateHandler(scope.row,'cross','','','')">答辩评审</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 8px 0 20px","margin":"0 4px 10px","borderColor":"#d7e0ea","color":"#466c95","outline":"none","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230220/9220697c21454c7f81dd6cc302220480.png) no-repeat 4px 8px,#eaeef2","borderWidth":"1px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('kaitidabian','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"cursor":"pointer","padding":"0 8px 0 20px","margin":"0 4px 10px","borderColor":"#edd9db","color":"#eb4a5f","outline":"none","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230220/afa1aa4f7bd9496e8301ffa4a0f01e20.png) no-repeat 4px 8px,#fbf2f2","borderWidth":"1px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('kaitidabian','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<dabianpingshen-cross-add-or-update v-if="dabianpingshenCrossAddOrUpdateFlag" :parent="this" ref="dabianpingshenCrossaddOrUpdate"></dabianpingshen-cross-add-or-update>
...
import dabianpingshenCrossAddOrUpdate from "../dabianpingshen/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dabianpingshenCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    dabianpingshenCrossAddOrUpdate,
  },
  methods: {
    ...
    dabianpingshenCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dabianpingshenCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','kaitidabian');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.dabianpingshenCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.dabianpingshenCrossaddOrUpdate.init(row.id,type);
      });
    },
