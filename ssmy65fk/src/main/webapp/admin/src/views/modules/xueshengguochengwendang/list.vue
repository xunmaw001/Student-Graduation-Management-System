		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"0 0 0 20px","background":"none","display":"flex","width":"100px","justifyContent":"space-between","order":"5"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0","borderColor":"#fde2db #fbccc0 #f58c71","margin":"0","textAlign":"center","display":"flex","justifyContent":"center","top":"180px","flexWrap":"wrap","left":"20px","background":"none","borderWidth":"0px","width":"calc(100% - 20px)","position":"absolute","borderStyle":"solid"}' >
					<div :style='{"width":"auto","margin":"0 10px 0 0","background":"none","display":"inline-block"}'>
						<label :style='{"padding":"0 4px 0 0","color":"#666","textAlign":"right","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">过程标题</label>
						<el-input v-model="searchForm.guochengbiaoti" placeholder="过程标题" clearable></el-input>
					</div>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('xueshengguochengwendang','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="guochengtupian" width="200" label="过程图片">
						<template slot-scope="scope">
							<div v-if="scope.row.guochengtupian">
								<img v-if="scope.row.guochengtupian.substring(0,4)=='http'" :src="scope.row.guochengtupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.guochengtupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
  ...
  methods: {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.guochengbiaoti!='' && this.searchForm.guochengbiaoti!=undefined){
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "xueshengguochengwendang/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
